...
/** 根据 id 获取图片（封装类 用户使用） GET /api/picture/get/vo */
...
/** 分页获取图片列表（封装类 用户使用） POST /api/picture/list/page/vo */
...
/** 分页Cache获取图片列表（用户使用） POST /api/picture/list/page/vo/cache */
export async function listPictureVoCacheByPageUsingPost(
  body: API.PictureQueryRequest,
  options?: { [key: string]: any }
) {
...
/** 审核图片（仅管理员可用） POST /api/picture/review */
...
/** 上传图片 by 文件 POST /api/picture/upload */
...
/** 批量抓取图片（仅管理员可用） POST /api/picture/upload/batch */
...
/** 上传图片 bu URL POST /api/picture/upload/url */
